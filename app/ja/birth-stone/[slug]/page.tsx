...
export async function generateStaticParams() {
  return getAllSlugs("ja", "birth-stone").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "ja", "birth-stone");
    const alternates = getAlternateLanguages(slug, "birth-stone");
    
    return {
      title: content.frontmatter.title || `${slug}の誕生石`,
      description: content.frontmatter.description || `${slug}の誕生石の意味とパワー`,
      alternates: {
...
export default async function BirthstonePage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "ja", "birth-stone");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/ja" className="text-amber-600 hover:text-amber-800">
            ← ホーム
          </Link>
        </nav>
