...
export async function generateStaticParams() {
  return getAllSlugs("en", "birth-flower").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "en", "birth-flower");
    const alternates = getAlternateLanguages(slug, "birth-flower");
    
    return {
      title: content.frontmatter.title || `${slug.charAt(0).toUpperCase() + slug.slice(1)} Birth Flower`,
    ...
    return { title: "Birth Flower Not Found" };
  }
}
...
export default async function BirthFlowerPage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "en", "birth-flower");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/en" className="text-pink-600 hover:text-pink-800">
            ← Home
          </Link>
        </nav>
