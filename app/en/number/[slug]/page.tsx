...
export async function generateStaticParams() {
  return getAllSlugs("en", "number").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const angelNumber = await readMarkdownFile(slug, "en", "number");
    const alternates = getAlternateLanguages(slug, "number");
...
export default async function AngelNumberPage({ params }: Props) {
  ...
  let angelNumber;
  try {
    angelNumber = await readMarkdownFile(slug, "en", "number");
  } catch {
    notFound();
  }
