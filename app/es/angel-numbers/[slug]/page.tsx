...
export async function generateStaticParams() {
  return getAllAngelNumberSlugs("es").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const dream = await readMarkdownFile(slug, "es");
    const alternates = getAlternateLanguages(slug);
    
    return {
      title: dream.frontmatter.title || `Significado de soñar con ${slug}`,
      description: dream.frontmatter.description || `Descubre el significado de soñar con ${slug}`,
      alternates: {
    ...
    return { title: "Sueño no encontrado" };
  }
}
...
export default async function DreamPage({ params }: Props) {
  ...
  let dream;
  try {
    dream = await readMarkdownFile(slug, "es");
  } catch {
    notFound();
  }
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="text-left">
              <p className="text-gray-700 text-lg">
                Tu sueño habló — deja que las cartas respondan
              </p>
              <p className="text-purple-600 font-medium flex items-center gap-1">
                Pregunta a las cartas <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
              </p>
            </div>
          </div>
        </a>
