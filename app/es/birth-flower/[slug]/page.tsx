...
export async function generateStaticParams() {
  return getAllSlugs("es", "birth-flower").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "es", "birth-flower");
    const alternates = getAlternateLanguages(slug, "birth-flower");
    
    return {
      title: content.frontmatter.title || `Flor de Nacimiento de ${slug}`,
      description: content.frontmatter.description || `Significado de la flor de nacimiento de ${slug}`,
      alternates: {
    ...
    return { title: "Flor de Nacimiento No Encontrada" };
  }
}
...
export default async function BirthFlowerPage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "es", "birth-flower");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/es" className="text-pink-600 hover:text-pink-800">
            ← Inicio
          </Link>
        </nav>
