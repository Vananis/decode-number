...
export async function generateStaticParams() {
  return getAllSlugs("es", "birth-stone").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "es", "birth-stone");
    const alternates = getAlternateLanguages(slug, "birth-stone");
    
    return {
      title: content.frontmatter.title || `Piedra de Nacimiento de ${slug}`,
      description: content.frontmatter.description || `Significado de la piedra de nacimiento de ${slug}`,
      alternates: {
    ...
    return { title: "Piedra de Nacimiento No Encontrada" };
  }
}
...
export default async function BirthstonePage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "es", "birth-stone");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/es" className="text-amber-600 hover:text-amber-800">
            ← Inicio
          </Link>
        </nav>
