...
export async function generateStaticParams() {
  return getAllAngelNumberSlugs("zh-hant").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const dream = await readMarkdownFile(slug, "zh-hant");
    const alternates = getAlternateLanguages(slug);
    
    return {
      title: dream.frontmatter.title || `${slug} 解夢`,
      description: dream.frontmatter.description || `${slug} 夢境的意義與解讀`,
      alternates: {
...
export default async function DreamPage({ params }: Props) {
  ...
  let dream;
  try {
    dream = await readMarkdownFile(slug, "zh-hant");
  } catch {
    notFound();
  }
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="text-left">
              ...
              <p className="text-purple-600 font-medium flex items-center gap-1">
                Ask the cards <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
              </p>
            </div>
          </div>
        </a>
