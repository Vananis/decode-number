...
export async function generateStaticParams() {
  return getAllSlugs("zh-hant", "birth-flower").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "zh-hant", "birth-flower");
    const alternates = getAlternateLanguages(slug, "birth-flower");
    
    return {
      title: content.frontmatter.title || `${slug}誕生花`,
      description: content.frontmatter.description || `${slug}誕生花的含義與花語`,
      alternates: {
...
export default async function BirthFlowerPage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "zh-hant", "birth-flower");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/zh-hant" className="text-pink-600 hover:text-pink-800">
            ← 首頁
          </Link>
        </nav>
