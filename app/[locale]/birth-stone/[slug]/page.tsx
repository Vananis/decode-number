...
export function generateStaticParams() {
  ...
  for (const locale of supportedLocales) {
    const slugs = getAllSlugs(locale, 'birth-stone');
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const content = await readMarkdownFile(slug, locale, 'birth-stone');
    const alternates = getAlternateLanguages(slug, 'birth-stone');
    
    return {
      title: content.frontmatter.title || `${slug} Birthstone`,
      description: content.frontmatter.description || `Discover the birthstone for ${slug}`,
      alternates: {
...
export default async function BirthStonePage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, locale, 'birth-stone');
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href={`/${locale}`} className="text-amber-600 hover:text-amber-800">
            {dict.nav.home}
          </Link>
        </nav>
