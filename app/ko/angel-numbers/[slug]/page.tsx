...
export async function generateStaticParams() {
  return getAllAngelNumberSlugs("ko").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const dream = await readMarkdownFile(slug, "ko");
    const alternates = getAlternateLanguages(slug);
    
    return {
      title: dream.frontmatter.title || `${slug} 꿈 해몽`,
      description: dream.frontmatter.description || `${slug} 꿈을 꿨을 때 의미와 해석`,
      alternates: {
...
export default async function DreamPage({ params }: Props) {
  ...
  let dream;
  try {
    dream = await readMarkdownFile(slug, "ko");
  } catch {
    notFound();
  }
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="text-left">
              ...
              <p className="text-purple-600 font-medium flex items-center gap-1">
                카드에게 물어보기 <span className="inline-block transition-transform group-hover:translate-x-1">→</span>
              </p>
            </div>
          </div>
        </a>
