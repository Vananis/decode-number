...
export async function generateStaticParams() {
  return getAllSlugs("ko", "birth-flower").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "ko", "birth-flower");
    const alternates = getAlternateLanguages(slug, "birth-flower");
    
    return {
      title: content.frontmatter.title || `${slug} 탄생화`,
      description: content.frontmatter.description || `${slug} 탄생화의 의미와 꽃말`,
      alternates: {
...
export default async function BirthFlowerPage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "ko", "birth-flower");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/ko" className="text-pink-600 hover:text-pink-800">
            ← 홈
          </Link>
        </nav>
