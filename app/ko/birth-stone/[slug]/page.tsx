...
export async function generateStaticParams() {
  return getAllSlugs("ko", "birth-stone").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  try {
    const content = await readMarkdownFile(slug, "ko", "birth-stone");
    const alternates = getAlternateLanguages(slug, "birth-stone");
    
    return {
      title: content.frontmatter.title || `${slug} 탄생석`,
      description: content.frontmatter.description || `${slug} 탄생석의 의미와 효능`,
      alternates: {
...
export default async function BirthstonePage({ params }: Props) {
  ...
  let content;
  try {
    content = await readMarkdownFile(slug, "ko", "birth-stone");
  } catch {
    notFound();
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      <div className="max-w-3xl mx-auto px-4 py-16">
        <nav className="mb-8">
          <Link href="/ko" className="text-amber-600 hover:text-amber-800">
            ← 홈
          </Link>
        </nav>
